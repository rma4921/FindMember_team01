...
public interface RecruitmentRepository extends JpaRepository<Recruitment, Long> {
	...
	Page<Recruitment> findByTitleContainingOrContentContaining(String title, String content, Pageable pageable);

	@Query("SELECT r FROM Recruitment r WHERE (r.title LIKE %:keyword% OR r.content LIKE %:keyword%)")
	Page<Recruitment> findByKeyword(@Param("keyword") String keyword, Pageable pageable);

	@Query("SELECT r FROM Recruitment r WHERE r.status = :status AND (r.title LIKE %:keyword% OR r.content LIKE %:keyword%)")
	Page<Recruitment> findByStatusAndKeyword(@Param("status") int status, @Param("keyword") String keyword,
		Pageable pageable);
}
